...
struct PluginLoadFailureSurfacingTests {
    ...
    func failureTextIsReadable() {
        let text = AppStoreStore.failureText(PluginLoadFailure(
            ...
            reason: "was built against a newer AinkradAppKit than this host embeds; bump the host's SDK pin or rebuild the plugin (missing symbol _$s21AinkradAppKitContract15MCPResourceSpecV012requiresLiveB0Sbvs)"))
        #expect(text.hasPrefix("GitMage — was built against a newer AinkradAppKit"))
