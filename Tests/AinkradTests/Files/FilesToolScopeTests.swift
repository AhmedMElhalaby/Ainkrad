...
struct FilesToolScopeTests {
    ...
    func outsideRootsRefused() {
        let decision = FilesToolScope.decide(target: url("/Users/test/other/thing.txt"),
    ...
    func explicitAbsoluteAllowed() {
        #expect(FilesToolScope.decide(target: url("/Users/test/other/thing.txt"),
    ...
    func systemDirectoriesAlwaysRefused() {
        for path in ["/System/Library/x", "/usr/bin/git", "/bin/sh", "/Library/Preferences/x"] {
            #expect(FilesToolScope.decide(target: url(path), openRoots: roots,
    ...
    func filesystemRootRefused() {
        #expect(FilesToolScope.decide(target: url("/"), openRoots: roots,
                                      wasExplicitlyAbsolute: true) != .allowed)
    }
    ...
    func traversalRefused() {
        ...
        #expect(FilesToolScope.decide(target: target, openRoots: roots,
                                      wasExplicitlyAbsolute: true) != .allowed)
    }

    @Test("with no panes open, only explicit absolute paths are reachable")
    func noOpenPanes() {
        #expect(FilesToolScope.decide(target: url("/Users/test/a.txt"), openRoots: [],
    ...
    func siblingPrefixNotInside() {
        #expect(!FilesToolScope.isInside("/Users/test/project-secrets/x", anyOf: roots))
        #expect(FilesToolScope.isInside("/Users/test/project/x", anyOf: roots))
    }
}
