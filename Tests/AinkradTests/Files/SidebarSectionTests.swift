...
@MainActor
@Suite("Pinned roots")
struct FilesPinnedRootsTests {
    private func makePins() -> FilesPinnedRoots {
        FilesPinnedRoots(persistence: InMemoryPersistenceStore())
    }
    ...
    func persists() {
        ...
        let first = FilesPinnedRoots(persistence: persistence)
        first.pin(url("/work/thing"))

        let second = FilesPinnedRoots(persistence: persistence)
