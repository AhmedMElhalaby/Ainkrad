...
@MainActor
@Suite("FilesPaneStore")
struct FilesPaneStoreTests {
    ...
    func initialTab() {
        let store = FilesPaneStore(fileSystem: makeFS(), persistence: MemoryPersistence())
    ...
    func persistence() {
        ...
        let store = FilesPaneStore(fileSystem: makeFS(), persistence: persistence)
        store.newTab(at: home.appendingPathComponent("Documents"))
        store.activeTab.showHidden = true
        store.persist()

        let restored = FilesPaneStore(fileSystem: makeFS(), persistence: persistence)
