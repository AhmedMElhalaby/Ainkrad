...
/// Replays the transcript's `scry_render` tool calls to rebuild a `ScryModel`
...
enum ScryReconstruction {
    static func rebuild(from messages: [AgentMessage], toolName: String = "scry_render") -> ScryModel {
        var model = ScryModel()
    ...
    static func apply(_ input: JSONValue, to model: inout ScryModel) {
    ...
    /// `scry_render` tool implementation). Synthesizes an id if the input
    /// omits one, so every decoded element has stable identity.
    static func element(from input: JSONValue) -> ScryElement? {
    ...
    static func element(from input: JSONValue, fallbackID: String) -> ScryElement? {
        let kindRaw = input["kind"]?.stringValue ?? "card"
        let kind = ScryElementKind(rawValue: kindRaw) ?? .unknown
        return ScryElement(
    ...
    private static func merge(_ input: JSONValue, into e: inout ScryElement) {
        if let k = input["kind"]?.stringValue { e.kind = ScryElementKind(rawValue: k) ?? .unknown }
    ...
    private static func rect(from input: JSONValue) -> ScryRect {
        var r = ScryRect.defaultCard
