...
enum IslandMotionMath {
    /// Horizontal-only drift for a cloud, bounded to ±52 pt. `seed` (0,1,2,…)
    ...
    static func cloudDriftX(seed: Int, time: Double) -> CGFloat {
        let s = Double(seed)
        let speed = 0.30 + (s * 0.043).truncatingRemainder(dividingBy: 0.22)   // 0.30–0.52 rad/s
        let phase = (s * 1.7).truncatingRemainder(dividingBy: 2 * .pi)
        let amplitude = 32 + (s * 6.7).truncatingRemainder(dividingBy: 20)      // 32–52 pt
        return CGFloat(sin(time * speed + phase) * amplitude)
    }

    /// Vertical-only bob for an islet, bounded to ±18 pt. Varied per seed.
    static func isletBobY(seed: Int, time: Double) -> CGFloat {
        let s = Double(seed)
        let speed = 0.6 + (s * 0.087).truncatingRemainder(dividingBy: 0.5)      // 0.6–1.1 rad/s
        let phase = (s * 2.3).truncatingRemainder(dividingBy: 2 * .pi)
        let amplitude = 10 + (s * 3.3).truncatingRemainder(dividingBy: 8)       // 10–18 pt
        return CGFloat(sin(time * speed + phase) * amplitude)
    }
}
