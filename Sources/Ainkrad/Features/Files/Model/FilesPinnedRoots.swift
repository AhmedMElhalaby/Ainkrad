...
/// Its own document rather than a field on `FilesSettingsDocument`: pins are
...
struct FilesPinnedRootsDocument: PersistableDocument {
...
/// every pane, so this lives in `AppEnvironment` beside the other Files
/// services rather than inside `FilesPaneStore`.
@MainActor
@Observable
final class FilesPinnedRoots {
    ...
    init(persistence: PersistenceStore) {
        ...
        let document = persistence.load(FilesPinnedRootsDocument.self)
    ...
    private func persist() {
        persistence.save(FilesPinnedRootsDocument(paths: roots.map(\.path)))
    }
}
