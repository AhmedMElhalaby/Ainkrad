...
/// The Assistant's own app-settings surface (rendered under BUILT-IN APPS in
/// `SettingsOverlayView`).
///
...
/// into standalone views in `AssistantSettingsView+Sections.swift` /
/// `+Connections.swift`. What went away is the nested pill bar, not the UI.
///
/// What remains here is what is genuinely app-specific: the Assistant's
/// appearance (surface opacity, blur, and its own message font/size).
struct AssistantSettingsView: View {
    // Note: not `private` — the appearance builder in
    // `AssistantSettingsView+Sections.swift` needs access, and Swift's
