...
/// The "LABEL over control" row idiom shared by the Sage's settings
/// sections. Previously a private `labeled(_:tokens:)` helper on
/// `SageSettingsView`; now a standalone view, because those sections are
/// standalone views themselves.
struct SageSettingsLabeled<Content: View>: View {
...
extension SageSettingsView {
    ...
    /// used to live on `SageSettingsView`, so the settings catalog can
        ...
        var body: some View {
            ...
            return AinkradSettingsPanel(title: "Permissions",
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    SageSettingsLabeled("Always-allowed tools", tokens: tokens) {
    ...
    func appearanceSection(tokens: DesignTokens) -> some View {
        ...
        return AinkradSettingsPanel(
            ...
            // Sage text: font family + size (assistant-only override; nil = inherit global)
